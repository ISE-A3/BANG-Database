...
CREATE or ALTER PROCEDURE dbo.usp_Afbeelding_Delete
	@AFBEELDING_BESTANDSNAAM VARCHAR(256) = NULL
AS
BEGIN  
	...
	BEGIN TRY
		...
		IF @AFBEELDING_BESTANDSNAAM IS NULL
			DELETE FROM AFBEELDING 
				WHERE AFBEELDING_BESTANDSNAAM NOT IN (SELECT AFBEELDING_BESTANDSNAAM FROM PUBQUIZ) 
					AND AFBEELDING_BESTANDSNAAM NOT IN (SELECT AFBEELDING_BESTANDSNAAM FROM PUBQUIZRONDE) 
					AND AFBEELDING_BESTANDSNAAM NOT IN (SELECT AFBEELDING_BESTANDSNAAM FROM VRAAG) 
					AND AFBEELDING_BESTANDSNAAM NOT IN (SELECT AFBEELDING_BESTANDSNAAM FROM VRAAGONDERDEEL)

		IF @AFBEELDING_BESTANDSNAAM IS NOT NULL
		BEGIN
		--checks hier
