...
CREATE OR ALTER PROCEDURE dbo.usp_Vraagonderdeel_Update
	...
	@HUIDIG_VRAAGONDERDEELNUMMER INT,
	@NIEUW_VRAAGONDERDEELNUMMER INT = NULL,
...
BEGIN  
	...
	BEGIN TRY
		...
		IF NOT EXISTS (
			...
			AND VO.VRAAGONDERDEELNUMMER = @HUIDIG_VRAAGONDERDEELNUMMER
			)
		...
		SET	VRAAGONDERDEELNUMMER = ISNULL (@NIEUW_VRAAGONDERDEELNUMMER, VRAAGONDERDEELNUMMER),
			VRAAGONDERDEEL = ISNULL (@VRAAGONDERDEEL, VRAAGONDERDEEL),
			VRAAGSOORT = ISNULL (@VRAAGSOORT, VRAAGSOORT)
		WHERE VRAAGONDERDEELNUMMER = @HUIDIG_VRAAGONDERDEELNUMMER
		AND VRAAG_ID = (
			SELECT VRAAG_ID
			FROM VRAAG
			WHERE VRAAG_NAAM = @VRAAG_NAAM
			)
		AND (@NIEUW_VRAAGONDERDEELNUMMER IS NOT NULL OR
			@VRAAGONDERDEEL IS NOT NULL OR
