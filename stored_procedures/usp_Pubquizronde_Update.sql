...
BEGIN  
	...
	BEGIN TRY
		...
		IF EXISTS(SELECT '' FROM PUBQUIZRONDE PR INNER JOIN EVENEMENT E ON PR.EVENEMENT_ID = E.EVENEMENT_ID
				WHERE E.EVENEMENT_NAAM = @EVENEMENT_NAAM AND PR.RONDENUMMER = @RONDE_NUMMER_NIEUW AND PR.RONDENUMMER != @RONDE_NUMMER_OUD)
		...
		IF(@RONDENAAM IS NULL)
		UPDATE PUBQUIZRONDE
		SET RONDENUMMER = @RONDE_NUMMER_NIEUW, THEMA = @THEMA, AFBEELDING_BESTANDSNAAM = @AFBEELDING
		...
		ELSE 
		UPDATE PUBQUIZRONDE
		SET RONDENUMMER = @RONDE_NUMMER_NIEUW, THEMA = @THEMA, RONDENAAM = @RONDENAAM, AFBEELDING_BESTANDSNAAM = @AFBEELDING
		WHERE EVENEMENT_ID = (SELECT EVENEMENT_ID FROM EVENEMENT WHERE EVENEMENT_NAAM = @EVENEMENT_NAAM) AND RONDENUMMER = @RONDE_NUMMER_OUD
