...
CREATE or ALTER PROCEDURE dbo.usp_Antwoord_Update
	@VRAAG_NAAM VARCHAR(256),
	@VRAAGONDERDEELNUMMER INT,
	@OUD_ANTWOORD VARCHAR(256),
	@NIEUW_ANTWOORD VARCHAR(256) = NULL,
	@PUNTEN INT = NULL
AS
BEGIN  
	...
	BEGIN TRY
		...
		UPDATE ANTWOORD
		SET	ANTWOORD = ISNULL (@NIEUW_ANTWOORD, ANTWOORD),
			PUNTEN = ISNULL (@PUNTEN, PUNTEN)
		WHERE ANTWOORD = @OUD_ANTWOORD
		AND VRAAGONDERDEEL_ID = (
			SELECT VRAAGONDERDEEL_ID
			FROM VRAAGONDERDEEL
		...
		AND (@NIEUW_ANTWOORD IS NOT NULL OR
			@PUNTEN IS NOT NULL)
